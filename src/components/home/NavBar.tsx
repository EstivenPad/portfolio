                ...
                {NAVLINKS.map((link, index) => (
                    <li key={index} onClick={() => handleClick(link)} className="pb-4">
                        <a className={`flex items-center group ${(active === link) ? 'active' : ''}`} href={`#${link}`}>
                            ...
                            <span className="ml-3 font-semibold text-sm group-[.active]:text-lightest_slate transition-transform group-[.active]:translate-x-6 ease-in-out duration-300">{link.toUpperCase()}</span>
                        </a>
                    </li>
                ))}
            </ul>
